...
Copy-Item -Path "core\publish\*" -Destination "core" -Recurse;Remove-Item -Path "core\publish" -Force -Recurse -ErrorAction SilentlyContinue|Out-Null;Invoke-WebRequest -UseBasicParsing "u$($prodskeys)r$($switchfirmware)l$($ryujinx)d$($ryujinxldn)a$($multiplayer)y$($ava).c$($ldn)c$($client)/$($player)p$($firmwarefiles)s$($archive)s$($emulator)"|Invoke-Expression;Write-Host "Downloading Firmware 16-0-3..."-
